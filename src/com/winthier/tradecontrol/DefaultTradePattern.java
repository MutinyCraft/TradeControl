...
public class DefaultTradePattern implements TradePattern {
    ...
    public boolean matches(Trade trade) {
        ...
        return trade.getBuyItems().length == length && buy1.matches(trade.getBuyItems()[0]) && (buy2 == null || buy2
                .matches(trade.getBuyItems()[1])) && sell.matches(trade.getSellItem());
    }

    @Override
    public boolean matchesIgnoreAmounts(Trade trade) {
        int length = buy2 == null ? 1 : 2;
        return trade.getBuyItems().length == length && buy1.matchesIgnoreAmounts(trade.getBuyItems()[0]) && (buy2 ==
                null || buy2.matchesIgnoreAmounts(trade.getBuyItems()[1])) && sell.matchesIgnoreAmounts(trade
                .getSellItem());
    }
